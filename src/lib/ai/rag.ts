 ...
 * 벡터 검색 채택 하한.
 *
 * 0.65였을 때 "가장 처음쓴일기" 같은 **메타 질문**에 전혀 무관한 일기가 관련 칩으로
 * 붙었다. 0.65가 잡음 구간 한가운데였기 때문이다.
 *
 * 실측(2026-09-22, 운영 51건 / `rag-similarity.manual.test.ts`):
 *   메타 질문 최상위   0.590 ~ 0.662   ("가장 처음쓴일기" → 0.662 "첫 피티")
 *   내용 질문 정답     0.681 ~ 0.832   ("성수동 서울숲" → 0.832 정확히 그 일기)
 * 둘 사이가 0.662~0.681로 좁아서 그 사이인 0.68로 잡았다. 더 올리면 "영어 학원
 * 상담"(0.681) 같은 정답이 떨어져 나간다.
 */
export const MIN_VECTOR_SIMILARITY = 0.68;
...
//
// 두 번째 묶음(메타어)은 2026-09-23에 추가했다. "가장 처음 쓴 일기는?"에서
// ["가장","처음","일기"]가 뽑혀 본문에 그 단어가 있는 무관한 일기가 관련 칩으로
// 붙었다(7월 19일 삼척 ← "처음"). 빈도 문제가 아니라서("일기"는 51건 중 1건뿐)
// DF 필터로는 안 걸린다 — **질문이 무엇을 묻는지 가리키는 말**이지 일기의 주제가
// 아니라서 거른다. 이런 말의 의미는 벡터 경로가 담당한다.
const QUESTION_STOPWORDS = new Set([
  ...
  // 메타어 — 기록 자체를 가리키는 말
  "일기", "기록", "메모", "내용", "얘기", "이야기", "썼어", "썼지", "적었어", "적었지",
  // 순서·정도 — 고유명사가 아니라 순서를 묻는 말
  "가장", "제일", "처음", "최초", "마지막", "최근", "예전", "옛날", "전부", "모두",
  "몇개", "개수", "얼마나",
]);
